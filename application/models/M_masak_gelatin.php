...
class M_masak_gelatin extends CI_Model
{
    ...
    public function add($data)
    {
        ...
        $sql = "
        INSERT INTO `tb_masak_gelatin`(`id_ts_melt`, `tgl_masak`, `shift`, `batch_masak`,`id_mm`,`jml_bahan`,`jml_air`,`temp_pel`,`jam_gel`,`jam_bt`,`mixing1`,`mixing2`,`vac1`,`vac2`,`scala_vac`,`visco_cps`,`visco_c`,`op1`,`op2`,`supervisor`,`created_at`, `created_by`, `updated_at`, `updated_by`, `is_deleted`) 
        VALUES ('$data[id_transaksi]','$data[tgl_masak]','$data[shift]','$data[batch_masak]','$data[id_mm]','$data[jml_bahan]','$data[jml_air]','$data[temp_pel]','$data[jam_gel]','$data[jam_bt]','$data[mixing1]','$data[mixing2]','$data[vac1]','$data[vac2]','$data[scala_vac]','$data[visco_cps]','$data[visco_c]','$data[op1]','$data[op2]','$data[supervisor]',NOW(),'$id_user','0000-00-00 00:00:00','','0')
        ";

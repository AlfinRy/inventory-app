...
class Permintaan_barang_gudang extends CI_Controller
{
    ...
    function __construct()
    {
        parent::__construct();
        // check_not_login();
        $this->load->model('M_permintaan_barang_melting');
        $this->load->model('M_permintaan_barang_gudang');
        $this->load->model('M_transaksi_melting');
    ...
    public function index()
    {
        ...
        $this->template->load('template', 'content/permintaan_barang_gudang/permintaan_barang_gudang_data', $data);
        // print_r($data['bm']);
    }
    ...
    public function disetujui()
    {
        ...
        if ($respon) {
            header('location:' . base_url('Permintaan_barang_gudang') . '?alert=success&msg=Selamat anda berhasil Menyetujui Permintaan Barang Gudang');
        } else {
            header('location:' . base_url('Permintaan_barang_gudang') . '?alert=error&msg=Maaf anda gagal Menyetujui Permintaan Barang Gudang');
        }
    }
    ...
    public function ditolak()
    {
        ...
        if ($respon) {
            header('location:' . base_url('Permintaan_barang_gudang') . '?alert=success&msg=Selamat anda berhasil Menolak Permintaan Barang Gudang');
        } else {
            header('location:' . base_url('Permintaan_barang_gudang') . '?alert=error&msg=Maaf anda gagal Menolak Permintaan Barang Gudang');
        }
    }
    ...
    public function delete($no_transfer_slip)
    {
        ...
        if ($respon) {
            header('location:' . base_url('permintaan_barang_gudang') . '?alert=success&msg=Selamat anda berhasil menghapus Permintaan Barang Masuk Gudang');
        } else {
            header('location:' . base_url('permintaan_barang_gudang') . '?alert=error&msg=Maaf anda gagal menghapus Permintaan Barang Masuk Gudang');
        }
    }

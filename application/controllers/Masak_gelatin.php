...
class Masak_gelatin extends CI_Controller
{
	...
	function __construct()
	{
		parent::__construct();
		// check_not_login();
		$this->load->model('M_masak_gelatin');
		$this->load->model('M_marketing/M_print_schedule');
		$this->load->model('M_barang_masuk_melting');
		$this->load->model('M_transaksi_melting');
		$this->load->model('M_alat_kalibrasi');
	}
	...
	public function add()
	{
		...
		if ($respon) {
			header('location:' . base_url('Masak_gelatin') . '?alert=success&msg=Selamat anda berhasil menambah Bahan Masak Gelatin');
		} else {
			header('location:' . base_url('Masak_gelatin') . '?alert=error&msg=Maaf anda gagal menambah Bahan Masak Gelatin');
		}
	}
	...
	public function update()
	{
		...
		if ($respon) {
			header('location:' . base_url('Masak_gelatin') . '?alert=success&msg=Selamat anda berhasil meng-update Bahan Masak Gelatin');
		} else {
			header('location:' . base_url('Masak_gelatin') . '?alert=error&msg=Maaf anda gagal meng-update Bahan Masak Gelatin');
		}
	}
	...
	public function delete($batch_masak)
	{
		...
		if ($respon) {
			header('location:' . base_url('Masak_gelatin') . '?alert=success&msg=Selamat anda berhasil menghapus Bahan Masak Gelatin');
		} else {
			header('location:' . base_url('Masak_gelatin') . '?alert=error&msg=Maaf anda gagal menghapus Bahan Masak Gelatin');
		}
	}
}
